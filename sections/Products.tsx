...
export async function loader(_req: Request,) {
  const produtos = (await fetch(
    `http://localhost:3000/api/products/brazil`,
  ).then((r) => {
    return r.json();
...
export default function Products({ produtos }: SectionProps<typeof loader>) {
  return (
    <section class="lg:container mx-8 my-5 mt-20 text-xl md:text-base">
      <h1 class="mt-5 mb-10 text-2xl font-bold">Produtos selecionados para você</h1>
